...
import com.lefc.jambly.domain.MinusMinusExpression;
import com.lefc.jambly.domain.PlusPlusExpression;
...
/** CUP v0.10k generated parser.
  * @version Sun Mar 05 16:55:24 CET 2023
  */
public class parser extends java_cup.runtime.lr_parser {
  ...
  public int error_sym() {return 1;}
    ...
    public List<PrintText> error = new ArrayList<>();
    int cont_errori = 0;
    boolean fatal = false;
    Scanner scan = (Scanner) getScanner();
     ...
     public void report_error(String message, int line) {
         ...
         output.textAndLine(line, scan.getBuffer());
    ...
    public void report_error(String message, Object info) {
        ...
        try {
            PrintText output = new PrintText();
            output.textAndLine(scan.getCurrentPos(), scan.getLine(), scan.getBuffer());
            ...
            if (info == null) {
                ...
                if (output.getPos() >= scan.getPosLastSemicolon()) {
                    output.setString("");
                }
                error.add(output);
                return;
...
class CUP$parser$actions {
    ...
    //struttura che permette di recuperare una dichiarazione nel caso in cui, in precedenza,
    //ci sia stato un errore sintattico legato alla mancanza del ";"
    List <String> dichRecup = new ArrayList<>();
    HashMap <String,String> TabTrad = new HashMap<>();

    public void structProg(int levP, boolean flag, String trad) {
        if (levP == 1) {
            if (flag) {
                com.lefc.jambly.InterpreterRunner.TRANSLATION_RESULT = com.lefc.jambly.InterpreterRunner.TRANSLATION_RESULT.concat("\n.data\n\n");
            }
            com.lefc.jambly.InterpreterRunner.TRANSLATION_RESULT = com.lefc.jambly.InterpreterRunner.TRANSLATION_RESULT.concat(trad + "\n");
            for (String s : dichRecup) {
                com.lefc.jambly.InterpreterRunner.TRANSLATION_RESULT = com.lefc.jambly.InterpreterRunner.TRANSLATION_RESULT.concat(s + "\n");
            }
            flagP1 = false;
            return;
        }
    ...
    throws java.lang.Exception
    {
      ...
      switch (CUP$parser$act_num)
		...
		Obj arracc = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                      ...
                                      Record record = SymbolTable.retrieveVariableInsideScope(D);

                                      if(record.getTabRegArr().containsKey(arracc.testo)){                                         
                                            STArr = T+" "+"REGT "+"0("+record.getTabRegArr().get(arracc.testo)+")";                                       
                                      }else{ 
		...
		Obj nome = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                            ...
                            Record record = SymbolTable.retrieveVariableInsideScope(nome.testo); 

                            if(checkFlag){
                                parser.report_error(Err_War,nome);
                                checkFlag = false;                               
                                
                                record = new Record("INTEGER", "$s"+countReg);
                                SymbolTable.getTable().put(nome.testo, record);
                                countReg++;
                            }
                            nome.tipo = record.getType();
                            RESULT = nome;
...
String valueErr;
if (sintErr) {
    Record record = SymbolTable.retrieveVariableInsideScope(lhs.testo);
    record.setValue(assespr.testo);
    ...
    record.setType(lhs.tipo);
    vTrad.add(Traduzione.createReg(lhs.tipo, lhs.testo));
    ...
    if (lhs.testo.contains("[") && lhs.testo.contains("]")) {
        ...
        Record record = SymbolTable.retrieveVariableInsideScope(var); //ottengo l'indirzzo del record

        String num = lhs.testo.substring(lhs.testo.indexOf("[") + 1, lhs.testo.indexOf("]"));
        ...
        if (lhs.genere.equals("ARRAY_ACCESS_I")) {
            ...
            boolean fg = record.setArrayList(assespr.testo, ind);
            ...
            if (fg) {
                record.removeElement(ind + 1);
            }
        }

        if (record.getTabRegArr().containsKey(lhs.testo)) {
            if (lhs.tipo.equals("INTEGER")) {
              switch (assespr.genere) {
                case "ESPRESSIONE":
                ...
                case "VARIABILE":
                  ...
                  break;
                case "ARRAY_ACCESS":
                  if (record.getTabRegArr().containsValue(assespr.testo)) {
            ...
            String ArrActTStore = lhs.testo.substring(lhs.testo.indexOf("(") + 1, lhs.testo.length() - 1);
            UT = lhs.testo.substring(l + 1, l + 4);
            record.getTabRegArr().put(lhs.testo, ArrActTStore);
            ...
            if (flagWarn) {
                ...
                if (lhs.tipo.equals("INTEGER")) {
                    ...
                    if (assespr.genere.equals("ESPRESSIONE")) {
                        ...
                        Record rec2 = SymbolTable.retrieveVariableInsideScope(assespr.testo);
                        TRAD = lhs.testo.substring(0, l - 2) + lhs.testo.substring(l - 2).replace(UT, rec2.getRegister());
        ...
        Record record = SymbolTable.retrieveVariableInsideScope(lhs.testo); //ÃÂ¨ per ottenere SOLO il record

        if (record.getModifier().equals("null")) {
            if (checkFlag) {
                parser.report_error(Err_War, lhs);
                checkFlag = false;
            }
            ...
            if (flagWarn) {
                ...
                String APP = Traduzione.funct(assespr.genere, assespr.testo);
            if (assespr.genere.equals("ARRAY_ACCESS")) {
                    if (assespr.testo.startsWith("$t")) {
                        TRAD = APP + Traduzione.OpEspr(3, record.getType()) + " " +
                                record.getRegister() + ", $zero" + ", " + Traduzione.getRegFP();
                    } else {
                        TRAD = assespr.testo + "\n" + APP + Traduzione.OpEspr(3, record.getType()) + " " +
                                record.getRegister() + ", $zero" + ", " + Traduzione.getRegFP();
                    }
                } else {
                    TRAD = APP + Traduzione.OpEspr(3, record.getType()) + " " + record.getRegister() +
                ...
                if (lhs.tipo.equals("INTEGER")) {
                    ...
                    if (assespr.genere.equals("ESPRESSIONE")) {
                        ...
                        TRAD = assespr.testo.replace("RIS", record.getRegister());

                    } else if (assespr.genere.equals("VARIABILE")) {

                        Record rec2 = SymbolTable.retrieveVariableInsideScope(assespr.testo);
                        TRAD = Traduzione.OpEspr(3, record.getType()) + " " + record.getRegister() +
                                ", $zero" + ", " + rec2.getRegister();

                    } else if (assespr.genere.equals("ARRAY_ACCESS")) {

                        if (assespr.testo.startsWith("$t")) {
                            TRAD = Traduzione.OpEspr(3, record.getType()) + " " +
                                    record.getRegister() + ", $zero" + ", " + assespr.testo;
                        } else {
                            int l = assespr.testo.lastIndexOf(" $t");
                            String UT = assespr.testo.substring(l + 1, l + 4);
                            TRAD = assespr.testo + "\n" + Traduzione.OpEspr(3, record.getType()) + " " +
                                    record.getRegister() + ", $zero" + ", " + UT;
                        }

                    } else {
                        record.setValue(assespr.testo);
                        TRAD = Traduzione.OpEspr(3, record.getType()) + "I" + " " + record.getRegister() +
                                ", $zero" + ", " + assespr.testo;
                    }
                } else {
    ...
    RESULT = new Obj(TRAD, lhs.tipo);
		...
		Obj relespr = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                                                                ...
                                                                                CheckClass.isValidEqualsOrNotEqualsComparison(eqespr.tipo,relespr.tipo);
		...
		Obj addespr = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                                                                ...
                                                                                checkFlag = CheckClass.checkOpCond(relespr.tipo,addespr.tipo);
	...
	checkFlag = CheckClass.checkOpCond(relespr.tipo,addespr.tipo);
		...
		Obj multespr = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
		 String dfT = CheckClass.checkExpressionType(addespr.tipo,multespr.tipo);
                                                                                       ...
                                                                                       if(checkFlag){ 
                                                                                         ...
                                                                                         RESULT = new Obj(" ","ESPRESSIONE",((Scanner) parser.getScanner()).getLine());
                                                                                       }else{
                                                                                      ...
                                                                                      RESULT = new Obj(TRAD,dfT,"ESPRESSIONE",((Scanner) parser.getScanner()).getLine());
                                                                                      }
		...
		int multesprright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-0)).right;
		Obj multespr = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
		 String smT = CheckClass.checkExpressionType(addespr.tipo,multespr.tipo);
                                                                                      ...
                                                                                      RESULT = new Obj(TRAD,smT,"ESPRESSIONE",((Scanner) parser.getScanner()).getLine());
                                                                                      }
		...
		Obj unex = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                                                                   ...
                                                                                   if(unex.genere.equals("NUMERO")){
                                                                                        if(Integer.parseInt(unex.testo) == 0){
                                                                                            ...
                                                                                            parser.report_error("FATAL ERROR: NON E' POSSIBILE EFFETTUARE LA DIVISIONE PER 0!\n", null);
                                                                                        }
                                                                                   }else{
                                                                                     Record record = SymbolTable.getCurrRec(unex.testo);  
                                                                                     if(record.getValue().equals("0")){
                                                                                        parser.report_error("FATAL ERROR: NON E' POSSIBILE EFFETTUARE LA DIVISIONE PER 0!\n", null);
                                                                                        FlagSyn = true;
                                                                                     }
                                                                                   }

                                                                                   String dvT = CheckClass.checkExpressionType(multespr.tipo,unex.tipo);
                                                                                   ...
                                                                                   RESULT = new Obj(TRAD,dvT,"ESPRESSIONE",((Scanner) parser.getScanner()).getLine());
                                                                                  }
		...
		int unexright = ((java_cup.runtime.Symbol)CUP$parser$stack.elementAt(CUP$parser$top-0)).right;
		Obj unex = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
		  String mlT = CheckClass.checkExpressionType(multespr.tipo,unex.tipo);
                                                                                  ...
                                                                                  if(checkFlag){
                                                                                    ...
                                                                                    RESULT = new Obj(TRAD,mlT,"ESPRESSIONE",((Scanner) parser.getScanner()).getLine()); 
                                                                                  }
		...
		Obj postespr = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
            ...
            TRAD = new MinusMinusExpression().calculate(postespr, parser);
		...
		Obj postespr = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
            ...
            TRAD = new PlusPlusExpression().calculate(postespr, parser);
            RESULT = new Obj(TRAD,postespr.tipo);
		...
		Obj espr = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
                                                         ...
                                                         if(checkFlag){
                                                              ...
                                                              List<String> listValue = Arrays.stream(new int[10]).mapToObj(it -> String.valueOf(it)).collect(Collectors.toList());
                                                              record = new Record("INTEGER",listValue,100,"$s"+countReg);
                                                              SymbolTable.getTable().put(nome.testo, record);
                                                              countReg++;
                                                         }
                                                        ...
                                                        if(!espr.tipo.equals("INTEGER")){
                                                            ...
                                                            if(espr.genere.equals("NUMERO")){
                                                                ...
                                                                I1 = Integer.parseInt(recE.getValue().toString());
                                                                I2 = Integer.parseInt(record.getValue().toString());
                                                            }                                            
                                                        }
                                                         
                                                        if(I1 > I2){                                                                     
                                                                  parser.report_error("FATAL ERROR: L'INDICE SUPERA LA DIMENSIONE MASSIMA DEL VETTORE!\n", null);
                                                                  //SARÃ NECESSARIO USCIRE DAL PROGRAMMA!!!
                                                        } 
                                                         
                                                        
                                                         String prod = nome.testo+"["+espr.testo+"]";
                                                         if(!record.getTabRegArr().containsKey(prod)){
                                                           ...
                                                           if(nome.tipo.equals("DOUBLE")){
                                                            ...
                                                            if(nome.tipo.equals("INTEGER")){                                                              
                                                               ...
                                                               TRAD = "MULT "+"$t"+countRTempS+", "+recE.getRegister()+", "+4+"\n"+
                                                                   "ADD "+"$t"+countRTempS+", "+"$t"+countRTempS+", "+record.getRegister()+"\n"+
                                                               ...
                                                               countRTemp = countRTemp+2;
                                                                ...
                                                                TRAD = "MULT "+"$t"+countRTemp+", "+recE.getRegister()+", "+8+"\n"+
                                                                       ...
                                                                       "ADD.D "+"$f"+countRegFP+", "+"$f"+countRegFP+", "+record.getRegister()+"\n"+
                                                                       "TEMP"+", "+"0("+"$f"+countRegFP+")";                                                                   
                                                                countRegFP = countRegFP + 2;
		...
		Obj arracc = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                      ...
                                      if(record.getTabRegArr().containsKey(arracc.testo)){
                                         TRAD = (String) record.getTabRegArr().get(arracc.testo);                                       
                                      }else{
                                        record.getTabRegArr().put(arracc.testo,ArrActTemp);
		...
		Obj simname = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-1)).value;
                                                ...
                                                if(sintErr){
                                                    ...
                                                    Record record = SymbolTable.getCurrRec(simname.testo);
                                                                                                    
                                                    record.setType(typeDeclErr);
		...
		Obj vard = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                                      ...
                                                      String valueErr;
                                                      String number = vard.testo.substring(vard.testo.indexOf("=") + 2);
                                                      vard.testo = Support.getVar(vard.testo);

                                                      Record record = SymbolTable.getCurrRec(vard.testo);
                                                      record.setModifier("final");
		...
		Obj vard = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                                        ...
                                                        if(vard.tipo.equals("IDENTIFICATORE")){ //variabile priva di inizializzazione
                                                          ...
                                                          record = SymbolTable.getCurrRec(vard.testo);
                                                          record.setType(vard.tipo);
                                                          ...
                                                          record = SymbolTable.retrieveVariableInsideScope(VAR);
 ...
 UtilParser.newScope(); 
		...
		Obj vi = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                                                        ...
                                                                        if(vdi.tipo.equals("ARRAY")){
                                                                          ProdParser.arrayDeclarationHandler(vdi.testo, vi.testo);
                                                                        }else{
                                                                          Record record = SymbolTable.getCurrRec(vdi.testo);
                                                                          record.setValue(vi.testo); 
                                                                        }
 ...
 UtilParser.newScope(); 
		...
		Obj mod_opt = (Obj)((java_cup.runtime.Symbol) CUP$parser$stack.elementAt(CUP$parser$top-0)).value;
                                    ...
                                    ProdParser prodParser = new ProdParser();
                                    prodParser.modifierHandler(mod_opt.testo);
